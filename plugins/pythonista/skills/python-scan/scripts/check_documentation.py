...
def find_python_files(path: Path) -> Iterator[Path]:
    """Find Python files under path, matching the other python-scan analyzers."""
...
def build_issues(metrics: FileDocMetrics) -> List[Dict[str, Any]]:
    """Flatten a file's documentation metrics into python-scan's shared issue shape."""
    issues: List[Dict[str, Any]] = []

    if not metrics.has_module_docstring:
        issues.append({
            'file': metrics.file_path, 'line': 1, 'issue_type': 'missing_module_docstring',
            'description': 'Module docstring is missing',
            'suggestion': 'Add a module-level docstring describing its purpose',
            'severity': 'low',
        })
    ...
    for func in metrics.functions:
        if func.is_public and not func.has_docstring:
            issues.append({
                'file': metrics.file_path, 'line': func.line_number, 'name': func.name,
                'issue_type': 'missing_docstring',
                'description': f'{func.name} has no docstring',
                'suggestion': 'Add a docstring covering purpose, parameters, and return value',
                'severity': 'medium',
            })
        if func.is_public and (not func.has_return_type or func.num_params_with_types < func.num_params):
            hints = []
            ...
            issues.append({
                ...
                'description': f'{func.name}: {", ".join(hints)}',
    ...
    for cls in metrics.classes:
        if cls.is_public and not cls.has_docstring:
            issues.append({
                'file': metrics.file_path, 'line': cls.line_number, 'name': cls.name,
                'issue_type': 'missing_docstring',
                'description': f'{cls.name} has no docstring',
                'suggestion': 'Add a docstring covering purpose and usage',
                'severity': 'medium',
            })
...
def main():
    ...
    all_metrics = [analyze_file(f) for f in find_python_files(target)]

    if args.format == 'json':
        issues = [issue for metrics in all_metrics for issue in build_issues(metrics)]
        coverage = [metrics.to_dict() for metrics in all_metrics]
        print(json.dumps({'issues': issues, 'coverage': coverage}, indent=2))
    else:
        if not all_metrics:
