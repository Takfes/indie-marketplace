...
4. Measure current metrics with `python-scan`'s `analyze_multi_metrics.py` (`../python-scan/scripts/analyze_multi_metrics.py <path> --format json`).
...
4. **Before/after metrics** via `scripts/compare_metrics.py <before_file> <after_file>` (shells out to `python-scan`'s complexity and documentation analyzers).
...
Structural moves only -- full catalog with examples in `references/patterns.md`. Idiom-level patterns (guard clauses, dictionary dispatch, match statements, named boolean conditions) live in `python-patterns` instead of being re-described here.
...
`python-scan`'s `analyze_multi_metrics.py` combines complexipy + radon + maintainability index in a single report (`../python-scan/scripts/analyze_multi_metrics.py <path> --format json`). This skill's `scripts/compare_metrics.py` shells out to it (and to `python-scan`'s `check_documentation.py`) to diff before/after.
...
- **python-patterns** -- idiom-level cleanup: guard clauses, dictionary dispatch, match statements, named boolean conditions, naming conventions, magic-number extraction. Lowest regression risk. Use for "make this readable", "clean up naming."
- **python-refactor** (this skill) -- structural restructuring. OOP transformation, SOLID, complexity metrics, migration checklists, benchmark validation. Use for "refactor this module", "reduce complexity", "transform to OOP."

**Escalation path**: python-patterns → python-refactor (idiom-level to structural).

## Integration

- **python-scan** -- deterministic metrics baseline; this skill's Phase 1/4 measurements and `scripts/compare_metrics.py` shell out to its analyzers
