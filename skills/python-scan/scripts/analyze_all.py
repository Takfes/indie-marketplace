...
def generate_report(path: str, skip_duplicates: bool = False) -> dict:
    ...
    results['complexity'] = run_analyzer('analyze_multi_metrics.py', path)

    print("🔍 Checking documentation coverage...", file=sys.stderr)
    results['documentation'] = run_analyzer('check_documentation.py', path)
...
def print_text_report(report: dict):
    ...
    if summary['by_category'].get('documentation', 0) > 5:
        recommendations.append("• Add missing docstrings and type hints")
    if summary['by_category'].get('code_smells', 0) > 5:
        recommendations.append("• Address code smells - fix bare excepts, god classes")
...
Runs all analysis checks:
  ...
  - Code smells (bare excepts, god classes, long parameter lists, etc.)
