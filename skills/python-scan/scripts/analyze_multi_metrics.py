...
class FunctionMetrics:
    ...
    def risk_level(self) -> str:
        """Assess risk against the tuned pass/warn/fail bands (Thresholds defaults)."""
        if self.cyclomatic > 20 or self.cognitive > 25:
            return "HIGH"
        elif self.cyclomatic > 15 or self.cognitive > 20:
            return "MEDIUM"
        return "LOW"
...
@dataclass
class Thresholds:
    """Configurable thresholds for metrics (python-refactor's tuned pass/warn/fail bands)."""
    cyclomatic_warning: int = 15
    cyclomatic_error: int = 20
    cognitive_warning: int = 20
...
def build_issues(results: List[FileMetrics], thresholds: Thresholds) -> List[dict]:
    """Flatten complexity/maintainability findings into python-scan's shared issue shape."""
    issues: List[dict] = []

    for file_metrics in results:
        for func in file_metrics.functions:
            if func.risk_level == "LOW":
                continue
            issues.append({
                'file': func.file, 'line': func.line_start, 'name': func.name,
                'issue_type': 'high_complexity',
                'description': f'{func.name}: cyclomatic={func.cyclomatic}, cognitive={func.cognitive}',
                'suggestion': 'Extract method or reduce branching to lower complexity',
                'severity': 'high' if func.risk_level == 'HIGH' else 'medium',
            })

        if file_metrics.maintainability_index < thresholds.mi_warning:
            issues.append({
                'file': file_metrics.file_path, 'line': 1,
                'issue_type': 'low_maintainability',
                'description': f'Maintainability index {file_metrics.maintainability_index} '
                                f'(grade {file_metrics.health_grade})',
                'suggestion': 'Reduce complexity and increase documentation to raise the maintainability index',
                'severity': 'high' if file_metrics.maintainability_index < thresholds.mi_error else 'medium',
            })
...
def main():
    ...
    if args.format == "json" or args.output:
        output_data = {
            'issues': build_issues(results, thresholds),
